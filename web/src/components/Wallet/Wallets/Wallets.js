...
const walletDashboard = ({ wallets, priceUsd }) => {
  ...
  return (
    ...
    <Grid numColsSm={2} numColsLg={3} className="gap-6">
      {wallets.map((item) => (
        <Card key={item.title} decoration="top" decorationColor="green">
          <Flex justifyContent="start" className="space-x-4">
            ...
            <div className="truncate">
              ...
              <Metric className="truncate">${(parseFloat(item.amount)).toLocaleString('en-US')} USD</Metric>
              <Text className="truncate">${(parseFloat(item.amount * (priceUsd ? priceUsd : 1))).toLocaleString('en-US')} ARS</Text>
            </div>
          </Flex>
        </Card>
      ))}
      <Card key="total" decoration="top" decorationColor="green">
        <Flex justifyContent="start" className="space-x-4">
          <Icon
            icon={CashIcon}
            variant="light"
          ...
          <div className="truncate">
            <Text>Total</Text>
            <Metric className="truncate">${(parseFloat(totalWallet)).toLocaleString('en-US')} USD</Metric>
            <Text className="truncate">${(parseFloat(totalWallet * (priceUsd ? priceUsd : 1))).toLocaleString('en-US')} ARS</Text>
          </div>
        </Flex>
      </Card>
    </Grid>
